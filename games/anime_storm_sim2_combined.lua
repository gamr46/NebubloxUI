...
/_/ |_/\___/_.___/\__,_/_.___/_/\____/_/|_|  
    ...
    v1.1.0-optimized  |  Nebublox UI Library (Performance Enhanced)
    
    Author: Nebublox
    Github: https://github.com/Nebublox/NebubloxUI
...
local function detectDeviceTier()
	local success, result = pcall(function()
		...
		pcall(function()
			...
			if us and us.GameSettings then
				graphicsQuality = us.GameSettings.SavedQualityLevel or 10
			end
		end)
		
		if isMobile or graphicsQuality <= 3 then
			return "Low"
		elseif graphicsQuality <= 7 then
			return "Medium"
		else
			return "High"
		end
	end)
	return success and result or "Medium"
end
...
PerformanceSettings={
	EnableAcrylicBlur=(detectDeviceTier()~="Low"),
	EnableAnimations=true,
	MaxCachedImages=detectDeviceTier()=="Low"and 20 or 50,
	FrameThrottle=detectDeviceTier()=="Low"and 3 or 1,
},
ImageCache={},
CacheSize=0,
DefaultProperties={
...
function p.ConvertGifToMp4(v,x,z,A)
	-- SECURITY: API key should be externalized. Performance: Skip on low-end devices
	if p.DeviceTier == "Low" then return nil end
	
	local B = p.CloudConvertApiKey -- Configure externally via p.CloudConvertApiKey
	local C=x.."/"..z.."-"..A..".mp4"
	if not B then 
		warn("[NebubloxUI] CloudConvert API key not configured. Set p.CloudConvertApiKey to enable GIF conversion.")
		return nil 
	end
...
local P="NebubloxUI/"..A.."/assets"
if isfolder and makefolder then
if not isfolder"NebubloxUI"then makefolder"NebubloxUI"end
if not isfolder("NebubloxUI/"..A)then makefolder("NebubloxUI/"..A)end
...
warn(string.format("[ NebubloxUI.Creator ] Failed to load custom asset '%s': %s",_,tostring(ab)))
H:Destroy()
return
end
end)
...
if not aa then
warn("[ NebubloxUI.Creator ]  '"..tostring(identifyexecutor and identifyexecutor()or"unknown").."' doesnt support the URL Images. Error: "..tostring(ab))
H:Destroy()
end
elseif L==""then
H.Visible=false
...
return[[
{
    ...
    "version": "1.0.222",
    "main": "./dist/main.lua",
    "repository": "https://github.com/Nebublox/NebubloxUI",
    "discord": "https://discord.gg/nebublox",
    "author": "Nebublox",
...
local ak=a.load'l'.Init(nil,ag.NebubloxUI.ScreenGui.KeySystem)
...
local m=ag.NebubloxUI.Services[l.Type]
if m then
local p={}
...
local ag=a.load'l'.Init(nil,ae.NebubloxUI.ScreenGui.Popups)
...
warn"[ NebubloxUI.ConfigManager ] Window.Folder is not specified."
return false
end

ac=af
ad.Folder=ac.Folder
ad.Path="NebubloxUI/"..tostring(ad.Folder).."/config/"

if not isfolder("NebubloxUI/"..ad.Folder)then
makefolder("NebubloxUI/"..ad.Folder)
if not isfolder("NebubloxUI/"..ad.Folder.."/config/")then
makefolder("NebubloxUI/"..ad.Folder.."/config/")
end
end
...
warn"[ NebubloxUI.ConfigManager ] The config system doesn't work in the studio."
return nil
end
...
if ak then
if ac.Debug then print("[ NebubloxUI.ConfigManager ] AutoLoaded config: "..af)end
else
warn("[ NebubloxUI.ConfigManager ] Failed to AutoLoad config: "..af.." - "..tostring(al))
end
end)
end
end
...
Visible=false,
Active=false,
Parent=ak.NebubloxUI.DropdownGui,
AnchorPoint=Vector2.new(1,0),
},{
al.UIElements.Menu,
ah("UISizeConstraint",{
...
ap.Load(ak,ao.Content,ap.Elements,aj.Window,aj.NebubloxUI,function()
...
warn("[ NebubloxUI ] Failed to apply pending config for '"..aq.Flag.."': "..tostring(aw))
end
end)
end
end
...
local e=a.load'U'
e.Load(ao,ao.UIElements.ContainerFrame,e.Elements,Window,NebubloxUI,nil,e,an)
...
if ar.Folder then
if not isfolder("NebubloxUI/"..ar.Folder)then
makefolder("NebubloxUI/"..ar.Folder)
end
if not isfolder("NebubloxUI/"..ar.Folder.."/assets")then
makefolder("NebubloxUI/"..ar.Folder.."/assets")
end
...
return ar.UIElements.SideBarContainer.AbsoluteSize.X/(aq.NebubloxUI.UIScale or 1)
end
...
if not h then
warn("[ NebubloxUI.Window.Background ] Failed to download video: "..tostring(j))
return
end
end
...
if not h then
warn("[ NebubloxUI.Window.Background ] Failed to load custom asset: "..tostring(j))
return
end
warn"[ NebubloxUI.Window.Background ] VideoFrame may not work with custom video"
e=j
end
...
local m=0
local p=ar.UIElements.Main.Main.Topbar.Right.UIListLayout.AbsoluteContentSize.X/aq.NebubloxUI.UIScale
if l and h then
m=math.max(l.TextBounds.X/aq.NebubloxUI.UIScale,h.TextBounds.X/aq.NebubloxUI.UIScale)
else
m=l.TextBounds.X/aq.NebubloxUI.UIScale
end
if j then
m=m+(ar.IconSize/aq.NebubloxUI.UIScale)+(ar.UIPadding/aq.NebubloxUI.UIScale)+(4/aq.NebubloxUI.UIScale)
end
ar.UIElements.Main.Main.Topbar.Center.Position=UDim2.new(
0,
m+(ar.UIPadding/aq.NebubloxUI.UIScale),
0.5,
0
)
ar.UIElements.Main.Main.Topbar.Center.Size=UDim2.new(
1,
-m-p-((ar.UIPadding*2)/aq.NebubloxUI.UIScale),
1,
0
)
end)
...
ImageTransparency=ar.Transparent and aq.NebubloxUI.TransparencyValue or 0
},{
p
})
end
...
aq.NebubloxUI.TransparencyValue=u
...
ak(ar.UIElements.MainBar,.16,{Size=UDim2.new(1,-(8/(aq.NebubloxUI.UIScale or 1)),1,-52)},Enum.EasingStyle.Sine,Enum.EasingDirection.Out):Play()
end)
...
if aq.NebubloxUI.UseAcrylic then
...
aq.NebubloxUI:ToggleAcrylic(true)
end)
end)
end
function ar.Close(r)
...
aq.NebubloxUI:ToggleAcrylic(false)
...
ar.Destroyed=true
task.wait(0.4)
aq.NebubloxUI.ScreenGui:Destroy()
aq.NebubloxUI.NotificationGui:Destroy()
aq.NebubloxUI.DropdownGui:Destroy()
...
ar.Transparent=u
aq.NebubloxUI.Transparent=u

ar.UIElements.Main.Background.ImageTransparency=u and aq.NebubloxUI.TransparencyValue or 0
...
return aq.NebubloxUI.UIScale
end

function ar.SetUIScale(r,u)
aq.NebubloxUI.UIScale=u
ak(aq.NebubloxUI.UIScaleObj,.2,{Scale=u},Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
return ar
end
...
local r=a.load'V'
local u=a.load'W'
local v=r.Init(ar,aq.NebubloxUI,aq.Parent.Parent.ToolTips)
...
return v.New(z,aq.NebubloxUI.UIScale)
end
...
return u.New(z,ar.UIElements.SideBar.Frame,ar.Folder,aq.NebubloxUI.UIScale,ar)
end
...
local M=H.AbsoluteContentSize.X/aq.NebubloxUI.UIScale
local N=J.AbsoluteSize.X/aq.NebubloxUI.UIScale
...
local T=S.AbsoluteSize.X/aq.NebubloxUI.UIScale
if T<Q then
Q=T
P=S
end
...
Name="NebubloxUI/Notifications",
Parent=as,
IgnoreGuiInset=true,
})
aa.DropdownGui=ao("ScreenGui",{
Name="NebubloxUI/Dropdowns",
Parent=as,
IgnoreGuiInset=true,
})
ar(aa.ScreenGui)
...
if not isfolder"NebubloxUI"then
makefolder"NebubloxUI"
end
if aw.Folder then
